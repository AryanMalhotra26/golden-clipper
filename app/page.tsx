...
// ----------------------------------------------------
// • Drop this into a Next.js app as app/page.tsx or pages/index.tsx
// • Tailwind v4 compatible (global css should have: @import "tailwindcss";)
// • Framer Motion for tasteful animation
// • Low-key luxury palette with gold accents
"use client";
...
const BRAND = {
  ...
  phoneE164: "+15196198792", // your real number in E.164
    ...
    <div className={`min-h-screen ${Gold.bg} text-zinc-100`}>
      ...
      <nav className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-black/40 bg-black/30 border-b border-zinc-800">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-7 text-sm text-zinc-300">
            <a className="hover:text-zinc-50" href="#services">
              Services
            </a>
            {/* Gallery link removed */}
            <a className="hover:text-zinc-50" href="#hours">
              Hours
            </a>
            <a className="hover:text-zinc-50" href="#location">
              Location
            </a>
          ...
          <div className="flex items-center gap-2">
            <a
              href={tel}
              className={`hidden sm:inline-flex px-4 py-2 rounded-full ${Gold.btn} ${Gold.ring} shadow`}
            >
              Call
            </a>
            <a
              href="#booking"
              className="inline-flex px-4 py-2 rounded-full border border-zinc-700 bg-black/50 hover:bg-black/70"
            >
              Book Now
            </a>
          </div>
      ...
      <header id="home" className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-20 md:py-28">
          ...
          >
            Premium fades & classic cuts in the heart of Springbank. Walk-ins
            welcome.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 10 }}
      ...
      <section id="services" className="max-w-6xl mx-auto px-4 py-16">
        <div className="max-w-2xl">
          ...
          <p className="text-zinc-400 mt-2">
            Straightforward menu. Consistent results.
          </p>
        </div>
          ...
          {PRICES.map((item) => (
            ...
            >
              ...
              <p className="text-sm text-zinc-400 mt-2">
                Quality tools, clean finish, no rush.
              </p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-6 text-sm text-zinc-400">
          Prices shown before tax. Walk-ins welcome. Debit, credit, cash, and
          tap accepted.
        </div>
        ...
        <div className="mt-8">
          <a
            href="#booking"
            className={`inline-flex px-6 py-3 rounded-xl ${Gold.btn} ${Gold.ring} font-semibold`}
          >
            Book a Service
          </a>
        </div>
      ...
      <section id="hours" className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-5 max-w-2xl">
          {[
            ...
            <div
              key={h.d}
              className={`p-5 rounded-xl bg-black/50 border border-zinc-800 ${Gold.ring}`}
            >
              <div className="flex items-center justify-between">
                <span>{h.d}</span>
      ...
      <section id="location" className="max-w-6xl mx-auto px-4 pb-16">
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          <div
            className={`rounded-3xl p-6 bg-black/50 border border-zinc-800 ${Gold.ring} shadow`}
          >
            ...
            <div className="mt-6 flex gap-3">
              <a
                href={BRAND.mapUrl.replace("output=embed", "")}
                target="_blank"
                className={`inline-flex px-6 py-3 rounded-xl ${Gold.btn} ${Gold.ring} font-semibold`}
              >
                Get Directions
              ...
              <a
                href="#contact"
                className="inline-flex px-6 py-3 rounded-xl border border-zinc-700 bg-black/50 hover:bg-black/70"
              >
                Contact
              </a>
            </div>
          ...
          <div className="w-full h-64 md:h-[420px] rounded-3xl overflow-hidden border border-zinc-800 shadow">
            <iframe
              title="Map to Golden Clipper Barbershop"
      ...
      <section id="booking" className="px-4 pb-16">
        <div
          className={`max-w-6xl mx-auto rounded-3xl p-8 md:p-10 bg-black/60 border border-zinc-800 ${Gold.ring} shadow`}
        >
          <div className="grid lg:grid-cols-3 gap-8 items-center">
            <div className="lg:col-span-2">
              <h2 className="text-3xl md:text-4xl font-bold">Book your chair</h2>
              <p className="mt-2 text-zinc-300">
                Choose a service, pick a time, and you’re set. Prefer to walk in?
                Come by anytime during hours.
              </p>
              <div className="mt-6 grid sm:grid-cols-3 gap-3 max-w-lg">
                <a
                  href={tel}
                  className={`inline-flex justify-center px-6 py-3 rounded-xl ${Gold.btn} ${Gold.ring} font-semibold`}
                >
                  Call
                </a>
                <button
                  onClick={openWhatsApp}
                  className="inline-flex justify-center px-6 py-3 rounded-xl border border-zinc-700 bg-black/50 hover:bg-black/70"
                >
                  WhatsApp
                </button>
                <a
              ...
              <p className="text-sm text-zinc-400 mt-4">
                Same-day availability • Quick touch-ups • Clean fades guaranteed
              </p>
            </div>
            <div
              className={`rounded-2xl p-6 bg-black/40 border border-zinc-800 ${Gold.ring}`}
            >
              <h3 className="font-semibold text-lg">Quick Inquiry</h3>
              <p className="text-sm text-zinc-400">
                Tap to open a message with your details.
              </p>
              <div className="mt-4 grid gap-3">
                <Input
                  placeholder="Your name"
                  value={qName}
                  onChange={(e) => setQName((e.target as HTMLInputElement).value)}
                />
                <Input
                  placeholder="Phone (optional)"
                  value={qPhone}
                  onChange={(e) => setQPhone((e.target as HTMLInputElement).value)}
                />
                <Select
                  value={qService}
                ...
                <Textarea
                  placeholder="Preferred time, barber, notes"
                  rows={3}
                  value={qNotes}
                  onChange={(e) =>
                    setQNotes((e.target as HTMLTextAreaElement).value)
                  }
                />
                <div className="flex gap-3 pt-1">
                  <button
                    onClick={openWhatsApp}
                    className={`${Gold.btn} ${Gold.ring} inline-flex justify-center px-5 py-3 rounded-xl font-semibold`}
                  >
                    Send via WhatsApp
                  </button>
                  <a
                    href={tel}
                    className="inline-flex justify-center px-5 py-3 rounded-xl border border-zinc-700 bg-black/50 hover:bg-black/70"
                  >
                    Call Instead
                  </a>
                </div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-20">
        ...
        <div className="mt-6 grid md:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={i}
              className={`p-6 rounded-2xl bg-black/50 border border-zinc-800 ${Gold.ring}`}
            >
              {t}
      ...
      <footer id="contact" className="border-t border-zinc-800">
        <div className="max-w-6xl mx-auto px-4 py-10 grid md:grid-cols-3 gap-8">
          <div>
            <div className={`text-xl font-extrabold ${Gold.text}`}>
              Golden Clipper
            </div>
            <p className="text-zinc-400 mt-2">
              Clean cuts. Sharp fades. Walk-ins welcome.
            </p>
          </div>
          <div>
            <h4 className="font-semibold">Visit</h4>
            <address className="not-italic text-zinc-300 mt-2">
              {BRAND.address}
            </address>
            <a
          ...
          <div>
            ...
            <ul className="mt-2 text-zinc-300 space-y-1">
              ...
              <li>
                Instagram:{" "}
                <a href={BRAND.igUrl} className="underline" target="_blank">
                  @goldenclipper.to
                </a>
              </li>
            </ul>
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full px-3 py-2 rounded-lg bg-black/40 border border-zinc-800 ${Gold.ring} placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-[#c8a24a]/30 ${
        props.className ?? ""
      }`}
    />
  );
...
function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`w-full px-3 py-2 rounded-lg bg-black/40 border border-zinc-800 ${Gold.ring} focus:outline-none focus:ring-2 focus:ring-[#c8a24a]/30 ${
        props.className ?? ""
      }`}
    />
  );
